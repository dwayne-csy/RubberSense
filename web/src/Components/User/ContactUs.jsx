// RubberSense/Web/src/Components/User/ContactUs.jsx
...
const ContactUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/v1/contact`, formData, config);

      if (response.data.success) {
        setFormData({ name: user ? user.name : '', email: user ? user.email : '', subject: '', message: '' });
        setNotification({ show: true, message: response.data.message || 'Message sent successfully!', type: 'success' });
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .crs-fg input:disabled,
        .crs-fg input[readonly] {
          background: #F1F8E9;
          color: #558B2F;
          cursor: not-allowed;
          border-color: #DCEDC8;
      ...
      <div className="crs-root">
        {loading ? (
          <div className="crs-loading">
            <div className="crs-loading-spin" />
            <p style={{ color: '#4CAF50' }}>Loading...</p>
          </div>
        ) : (
          <>
            ...
            <div className="crs-wrap">
              ...
              <div className="crs-right">
                ...
                <form onSubmit={handleSubmit}>
                  <div className="crs-row">
                    <div className="crs-fg">
                      ...
                      <input
                        ...
                        readOnly={!!user}
                      />
                      {user && <div className="crs-field-note"></div>}
                    </div>
                  ...
                  <button type="submit" className="crs-btn" disabled={sending} style={{color:'#ffffff'}}>
                    {sending ? (
