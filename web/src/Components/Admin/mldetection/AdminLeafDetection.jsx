...
const AdminLeafDetection = () => {
  ...
  const captureImage = () => {
    ...
    c.toBlob(blob => {
      ...
      setAnalysisResult(null); setSuccessMessage(null); setError(null); setAiInsights(null);
    }, 'image/jpeg', 0.95);
  };
  ...
  const processSelectedFile = (file) => {
    ...
    setError(null); setSelectedImage(file); setAnalysisResult(null); setSuccessMessage(null); setAiInsights(null);
    const reader = new FileReader();
    reader.onload = e => setImagePreview(e.target.result);
    reader.readAsDataURL(file);
  };
  ...
  const handleReset = () => {
    ...
    setError(null); setSuccessMessage(null); setAiInsights(null); setShowAiInsights(false);
    if (fileInputRef.current) fileInputRef.current.value = '';
  };

  const getHealthGradient = (status) => {
    switch (status?.toLowerCase()) {
      case 'healthy':  return 'linear-gradient(135deg, #1b5e20 0%, #2e7d32 60%, #388e3c 100%)';
      case 'diseased': return 'linear-gradient(135deg, #b71c1c 0%, #c62828 60%, #d32f2f 100%)';
      default: return 'linear-gradient(135deg, #e65100 0%, #ef6c00 60%, #f57c00 100%)';
    }
  };
  ...
  const getTextureIcon = (texture) => {
    switch (texture?.toLowerCase()) {
      ...
      case 'moderate': return <TextureIcon sx={{ color: '#e65100' }} />;
      default:         return <WarningIcon sx={{ color: '#c62828' }} />;
    }
  };
                  ...
                  {showAiInsights && aiInsights && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: 0.05 }}>
                      <Paper elevation={0} sx={{
                        ...
                        <Box sx={{ position: 'absolute', right: -20, bottom: -20, width: 160, height: 160, borderRadius: '50%', background: 'rgba(255,255,255,0.05)' }} />
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2.5 }}>
                          <Box sx={{ width: 3, height: 20, borderRadius: 4, bgcolor: 'rgba(255,255,255,0.5)' }} />
                          <BiotechIcon sx={{ fontSize: 17 }} />
                          <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>AI-Powered Insights</Typography>
                          <Chip label="Powered by Groq" size="small"
                            sx={{ ml: 0.5, bgcolor: 'rgba(255,255,255,0.15)', color: 'white', fontSize: '0.65rem', fontWeight: 700 }} />
                        </Box>
                        ...
                        <Grid container spacing={2}>
                          <Grid item xs={12}>
                            <Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.1)', borderRadius: 2 }}>
                              <Typography variant="body2" sx={{ fontStyle: 'italic', lineHeight: 1.7 }}>
                                "{aiInsights.overallReport || 'Analysis complete.'}"
                              </Typography>
                            </Box>
                          </Grid>
                          ...
                          <Grid item xs={12}>
                            <Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.07)', borderRadius: 2 }}>
                              <Typography variant="caption" sx={{ opacity: 0.65, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.8, display: 'flex', alignItems: 'center', gap: 0.5, mb: 1 }}>
                                <LocalHospitalIcon sx={{ fontSize: 14 }} /> Diagnosis
                              </Typography>
                              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.9)' }}>
                                {aiInsights.diagnosis || 'No diagnosis available.'}
                              </Typography>
                            </Box>
                          </Grid>

                          {aiInsights.treatmentPlan?.length > 0 && (
                            <Grid item xs={12} md={6}>
                              <Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.07)', borderRadius: 2, height: '100%' }}>
                                <Typography variant="caption" sx={{ opacity: 0.65, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.8, display: 'flex', alignItems: 'center', gap: 0.5, mb: 1 }}>
                                  <HealingIcon sx={{ fontSize: 14 }} /> Treatment Plan
                                </Typography>
                                <List dense disablePadding>
                                  {aiInsights.treatmentPlan.map((item, idx) => (
                                    <ListItem key={idx} disableGutters sx={{ py: 0.3 }}>
                                      <ListItemIcon sx={{ minWidth: 26 }}>
                                        <CheckCircleIcon sx={{ color: '#a5d6a7', fontSize: 15 }} />
                                      </ListItemIcon>
                                      <ListItemText primary={item} primaryTypographyProps={{ variant: 'body2', sx: { color: 'rgba(255,255,255,0.88)' } }} />
                                    </ListItem>
                                  ))}
                                </List>
                              </Box>
                          ...
                          {aiInsights.preventionPlan?.length > 0 && (
                            <Grid item xs={12} md={6}>
                              <Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.07)', borderRadius: 2, height: '100%' }}>
                                <Typography variant="caption" sx={{ opacity: 0.65, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.8, display: 'flex', alignItems: 'center', gap: 0.5, mb: 1 }}>
                                  <AgricultureIcon sx={{ fontSize: 14 }} /> Prevention
                                </Typography>
                                <List dense disablePadding>
                                  {aiInsights.preventionPlan.map((item, idx) => (
                                    <ListItem key={idx} disableGutters sx={{ py: 0.3 }}>
                                      <ListItemIcon sx={{ minWidth: 26 }}>
                                        <CheckCircleIcon sx={{ color: '#ffcc80', fontSize: 15 }} />
                                      </ListItemIcon>
                                      <ListItemText primary={item} primaryTypographyProps={{ variant: 'body2', sx: { color: 'rgba(255,255,255,0.88)' } }} />
                                    </ListItem>
                                  ))}
                    ...
                    <CardContent sx={{ p: { xs: 3, md: 4 } }}>
                      <Grid container spacing={3} alignItems="center">
                        <Grid item xs={12} md={5}>
                          <Typography variant="caption" sx={{ opacity: 0.65, textTransform: 'uppercase', letterSpacing: 2, fontWeight: 700 }}>Health Status</Typography>
                          ...
                          <Grid container spacing={1.5}>
                            <Grid item xs={6}>
                              <Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.13)', borderRadius: 2 }}>
                                ...
                                <Typography variant="body1" sx={{ fontWeight: 800, mt: 0.3, lineHeight: 1.2 }}>{diseaseInfo.name || 'None'}</Typography>
                              </Box>
                            </Grid>
                            <Grid item xs={6}>
                              <Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.13)', borderRadius: 2 }}>
                                <Typography variant="caption" sx={{ opacity: 0.65 }}>Severity</Typography>
                                <Typography variant="body1" sx={{ fontWeight: 800, mt: 0.3, textTransform: 'capitalize' }}>{diseaseInfo.severity || '—'}</Typography>
                              </Box>
                            </Grid>
                          </Grid>
                        </Grid>
      ...
      <Dialog open={cameraOpen} onClose={() => setCameraOpen(false)} maxWidth="md" fullWidth
        ...
        <DialogTitle sx={{ bgcolor: '#1b5e20', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <PhotoCameraIcon />
            <Typography component="span" variant="h6" sx={{ fontWeight: 700 }}>Capture Leaf Sample</Typography>
          </Box>
